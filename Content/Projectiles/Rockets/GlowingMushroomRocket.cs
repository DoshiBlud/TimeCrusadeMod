...
namespace TimeCrusadeMod.Content.Projectiles.Rockets
{
	public class GlowingMushroomRocket : ModProjectile
	{
			...
			if (Projectile.owner == Main.myPlayer && Projectile.timeLeft <= 3) {
				...
				if (Math.Abs(Projectile.velocity.X) >= 4f || Math.Abs(Projectile.velocity.Y) >= 4f) {
					for (int i = 0; i < 2; i++) {
						...
						Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + -2f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.5f,
							Projectile.width - 8, Projectile.height - 8, ModContent.DustType<GlowingMushroomDustGlowed>(), 0f, 0f, 100);
						fireDust.scale *= 2f + Main.rand.Next(4) * 0.1f;
                        ...
                        if (Projectile.ai[0] == 20)
                        {
                            ...
                            Projectile.ai[0] = 0;
                            float speedX = -Projectile.velocity.X * Main.rand.NextFloat(.4f, .9f) + Main.rand.NextFloat(-4f, 4f);
                            float speedY = -Projectile.velocity.Y * Main.rand.Next(40, 90) * 0.01f + Main.rand.Next(-10, 9) * 0.4f; // This is Vanilla code, a little harder to comprehend. This is just here to teach you that you can convert vanilla code to more readable code sometimes.

                            // Spawn the Projectile.
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + speedX, Projectile.position.Y + speedY, speedX, speedY, ModContent.ProjectileType<TinyGlowingMushroom>(), (int)(Projectile.damage * 0.5), 0f, Projectile.owner, 0f, 0f);
                        }
                        ...
                        Dust smokeDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + -2f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.5f, Projectile.width - 8, Projectile.height - 8, ModContent.DustType<GlowingMushroomDust>(), 0f, 0f, 100, default, 0.5f);
						smokeDust.fadeIn = 0.5f + Main.rand.Next(2) * 0.2f;
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 30; i++) {
				Dust smokeDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<GlowingMushroomDust>(), 0f, 0f, 100, default, 1.5f);
				smokeDust.velocity *= 1.4f;
			}

			// Spawn a bunch of fire dusts.
			for (int j = 0; j < 20; j++) {
				Dust fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<GlowingMushroomDust>(), 0f, 0f, 100, default, 3.5f);
				fireDust.noGravity = true;
				fireDust.velocity *= 7f;
				fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<GlowingMushroomDust>(), 0f, 0f, 100, default, 1.5f);
