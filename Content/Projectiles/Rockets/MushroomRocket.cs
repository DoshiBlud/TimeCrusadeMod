...
{
	public class MushroomRocket : ModProjectile
	{
				...
				if (Math.Abs(Projectile.velocity.X) >= 8f || Math.Abs(Projectile.velocity.Y) >= 8f) {
					for (int i = 0; i < 2; i++) {
						...
						Dust fireDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + -2f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.5f,
							Projectile.width - 8, Projectile.height - 8, ModContent.DustType<MushroomDustGlowed>(), 0f, 0f, 100);
                        ...
                        {
                            Projectile.ai[0] = 0;
                            // calculate speeds here so they're in scope before we use them
                            float speedX = -Projectile.velocity.X * Main.rand.NextFloat(.4f, .9f) + Main.rand.NextFloat(-4f, 4f);
                            float speedY = -Projectile.velocity.Y * Main.rand.Next(40, 90) * 0.01f + Main.rand.Next(-10, 9) * 0.4f;
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + speedX, Projectile.position.Y + speedY, speedX, speedY, ModContent.ProjectileType<TinyMushroom>(), (int)(Projectile.damage * 0.5), 0f, Projectile.owner, 0f, 0f);
                        }
                        ...
                        Dust smokeDust = Dust.NewDustDirect(new Vector2(Projectile.position.X + -2f + posOffsetX, Projectile.position.Y + 1f + posOffsetY) - Projectile.velocity * 0.5f, Projectile.width - 8, Projectile.height - 8, ModContent.DustType<MushroomDust>(), 0f, 0f, 100, default, 0.5f);
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 30; i++) {
				Dust smokeDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<MushroomDust>(), 0f, 0f, 100, default, 1.5f);
				smokeDust.velocity *= 1.4f;
			}

			// Spawn a bunch of fire dusts.
			for (int j = 0; j < 20; j++) {
				Dust fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<MushroomDust>(), 0f, 0f, 100, default, 3.5f);
				fireDust.noGravity = true;
				fireDust.velocity *= 7f;
				fireDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<MushroomDust>(), 0f, 0f, 100, default, 1.5f);
