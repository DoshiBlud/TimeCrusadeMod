...
namespace TimeCrusadeMod.Content.NPCs.Axy
{
    ...
    public class Axy : ModNPC
    {
        ...
        {
            potionType = (ModContent.ItemType<UltraHealingPotion>());
        }
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                    ...
                    if (attackCounter <= 0 && Vector2.Distance(NPC.Center, targe.Center) >= 250 && Collision.CanHit(NPC.Center, 1, 1, targe.Center, 1, 1))
                    {
                        ...
                        int projectile = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, direction * 24, ModContent.ProjectileType<AxyLaser>(), 5, 0, Main.myPlayer);
                        Main.projectile[projectile].timeLeft = 50000;
                        Main.projectile[projectile].damage = 100;
                    SoundEngine.PlaySound(SoundID.Item50 with { Pitch = 0.08f });
                    attackCounter = 70;
                        NPC.netUpdate = true;
                    }
                if (attackCounters > 0)
                ...
                // If the attack counter is 0, this NPC is less than 12.5 tiles away from its target, and has a path to the target unobstructed by blocks, summon a projectile.
                if (attackCounters <= 0 && Vector2.Distance(NPC.Center, target.Center) >= 250 && Collision.CanHit(NPC.Center, 1, 1, target.Center, 1, 1))
                {
                    for (int crystal = 0; crystal < 10; crystal++)
                    {
                        Vector2 direction = (target.Center - NPC.Center).SafeNormalize(Vector2.UnitX);
                        direction = direction.RotatedByRandom(MathHelper.ToRadians(360));

                        int projectile = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, direction * 24, ModContent.ProjectileType<AxyCrystal>(), 5, 0, Main.myPlayer);
                        Main.projectile[projectile].timeLeft = 50000;
