...
namespace TimeCrusadeMod.Content.NPCs.Terrifier
{
    ...
    internal class TerrifierHead : WormHead
    {
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<SoulofDread>(), 1, 2, 4));
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (phase1 == 0)
                {
                    ...
                    if (player.dead || !player.active)
                    {
                        ...
                        NPC.EncourageDespawn(900000000);
                        return;
                    }
                    ...
                    switch (Act)
                        {
                            case 1:
                            {
                                ...
                                if (attackCounter <= 4 && Vector2.Distance(NPC.Center, player.Center) >= 200 && Collision.CanHit(NPC.Center, 1, 1, player.Center, 1, 1))
                                {
                                    ...
                                    attackCounter = 70;
                                    SoundEngine.PlaySound(SoundID.Item114 with { Pitch = 0.6f, PitchVariance = 0.2f, Volume = 0.3f });
                                    NPC.netUpdate = true;
                                }
                                ...
                                {
                                    Act = 5;
                                    SoundEngine.PlaySound(SoundID.Roar with { Pitch = 0.04f, PitchVariance = 0.2f });
                                }
                            }
                            break;
                            ...
                            case 2:
                            {
                                ...
                                if (++timer < 180) // Wait for the timer to reach 180
                                {
                                    break; // Ending the state early; timer not done yet.
                                }
                        ...
                        case 3:
                            {
                                ...
                                if (++timer < 250) // Wait for the timer to reach 180
                                {
                                    break; // Ending the state early; timer not done yet.
                                }
                        ...
                        case 5:
                            {
                                ...
                                if (attackCounter <= 4 && Vector2.Distance(NPC.Center, player.Center) >= 120 && Collision.CanHit(NPC.Center, 1, 1, player.Center, 1, 1))
                                {
                                ...
                                if (++timer < 350) // Wait for the timer to reach 180
                                {
                                    break; // Ending the state early; timer not done yet.
                                }
                                Act = 6;
                        ...
                        case 6:
                            {
                                ...
                                if (attackCounter <= 4 && Vector2.Distance(NPC.Center, player.Center) >= 120 && Collision.CanHit(NPC.Center, 1, 1, player.Center, 1, 1))
                                {
                                    ...
                                    {
                                        Vector2 direction = (player.Center - NPC.Center).SafeNormalize(Vector2.UnitX);
                                        direction = direction.RotatedByRandom(MathHelper.ToRadians(12));
        ...
        internal class TerrifierBody : WormBody
        {
            ...
            public override void HitEffect(NPC.HitInfo hit)
            {
                ...
                if ((float)NPC.life == (float)NPC.lifeMax * halfHealth)
                {
                    int frontGoreType = Mod.Find<ModGore>("TerrifierBody_Back").Type;
