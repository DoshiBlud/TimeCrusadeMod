...
namespace TimeCrusadeMod.Content.Items.Weapons.Summoner.Minions
{
    ...
    public class AxyFriendBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<Axypanion>()] > 0)
            {
    ...
    public class AxypanionStaff : ModItem
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void SetDefaults()
        {
            ...
            Item.buffType = ModContent.BuffType<AxyFriendBuff>();
            // No buffTime because otherwise the item tooltip would say something like "1 minute duration"
            Item.shoot = ModContent.ProjectileType<Axypanion>(); // This item creates the minion projectile
        }
    ...
    public class Axypanion : ModProjectile
    {
        public Vector2 FireVelocity { get; private set; }
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] >= 60f)
            {
                ...
                // Only spawn the firing projectile on the owning client and only if we have a target
                if (Main.myPlayer == Projectile.owner && foundTarget)
                {
                    // The direction the projectile will fire using the found target center
                    Vector2 shootDirection = (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX);
                    // The final velocity vector (FireVelocity is a Vector2 property in this class)
                    Vector2 shootVelocity = shootDirection + FireVelocity;

                    int type = ModContent.ProjectileType<AxyKnife>();
                    float typ = 1;
                    for (int i = 0; i < typ; i++)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), new Vector2(Projectile.Center.X - 4f, Projectile.Center.Y), shootVelocity, type, Projectile.damage, 460, Projectile.owner);
                    }
                }
            }
        }
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<AxyFriendBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<AxyFriendBuff>()))
            {
        ...
        private void GeneralBehavior(Player owner, out Vector2 vectorToIdlePosition, out float distanceToIdlePosition)
        {
            ...
            float minionPositionOffsetX = (12 + Projectile.minionPos * 32) * -owner.direction;
            idlePosition.X += minionPositionOffsetX; // Go behind the player

            // All of this code below this line is adapted from Spazzamini code (ID 388, aiStyle 66)
