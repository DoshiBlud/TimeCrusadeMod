...
{
	public class NeutralTorch : ModItem
	{
		...
		public override void SetDefaults() {
			...
			Item.DefaultToTorch(ModContent.TileType<Tiles.NeutralTorch>(), 0, false);
			Item.value = 50;
		}
		...
		public override void HoldItem(Player player) {
			...
			if (Main.rand.NextBool(player.itemAnimation > 0 ? 7 : 30)) {
				Dust dust = Dust.NewDustDirect(new Vector2(player.itemLocation.X + (player.direction == -1 ? -16f : 6f), player.itemLocation.Y - 14f * player.gravDir), 4, 4, ModContent.DustType<NeutralDust>(), 0f, 0f, 100);
		...
		public override void AddRecipes() {
			CreateRecipe(100)
				.AddIngredient(ModContent.ItemType<NeutralityFragments>(), 3)
