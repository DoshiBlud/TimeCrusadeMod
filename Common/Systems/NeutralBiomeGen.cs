...
namespace TimeCrusadeMod.Common.Systems
{
    public class NeutralBiomeGen : ModSystem
    {
        ...
        {
            int genIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Jungle Chests"));
            if (genIndex != -1)
            {
                tasks.Insert(genIndex + 1, new PassLegacy("Neutral Biome", (generationProgress, config) =>
                {
                    ...
                    WorldGen.TileRunner(2300, 700, stoneRadius, stoneRadius, (ushort)ModContent.TileType<NeutralStone>());
                    WorldGen.TileRunner(2300, 700, chiselRadius, chiselRadius, (ushort)ModContent.TileType<NeutralChisel>());
                    Point pointss = new Point(2300, 700);
                    WorldUtils.Gen(pointss, new Shapes.Circle(25, 25), Actions.Chain(new GenAction[]
                    {
                         new Actions.SetTile((ushort)ModContent.TileType<NeutralityCrystal>()),
                    }));
                }));                               
            }
        }
